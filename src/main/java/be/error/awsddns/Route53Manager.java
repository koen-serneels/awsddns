...
public class Route53Manager {
	...
	@Value("${awsddns.server.hostnames}")
	private List<String> ddnsServerHostnames;
	...
	public void Route53Manager() {
		LOG.debug("Initializing " + Route53Manager.class.getSimpleName() + ". AWSRegion: " + awsRegion + ", AWSAccessKey (first char): " + abbreviate(awsAccessKey, 4)
				+ ", AWSSecretKey (first char): " + abbreviate(awsSecretKey, 4) + ", AWSDDNS Hostname(s):" + join(ddnsServerHostnames));
		...
		ddnsServerHostedZoneId = getHostedZone(ddnsServerHostnames.iterator().next()).getId();
	...
	public void updateIpFromClient(String clientHostname, String clientIpAddress) {
		...
		try {
			...
			ChangeResourceRecordSetsRequest changeResourceRecordSetsRequest = new ChangeResourceRecordSetsRequest().withHostedZoneId(hostedZoneId).withChangeBatch(
					new ChangeBatch().withComment("DDNS client record - do not edit").withChanges(new Change().withAction("UPSERT").withResourceRecordSet(
	...
	public void updateServerIp() {
		...
		try {
			...
			LOG.debug("Updating hostname(s) " + join(ddnsServerHostnames) + " in zone " + ddnsServerHostedZoneId + " to " + "ip " + "address" + " " + ipAddress);
			for (String hostname : ddnsServerHostnames) {
				ChangeResourceRecordSetsRequest changeResourceRecordSetsRequest = new ChangeResourceRecordSetsRequest().withHostedZoneId(ddnsServerHostedZoneId)
						.withChangeBatch(new ChangeBatch().withComment("DDNS record - do not edit").withChanges(new Change().withAction("UPSERT").withResourceRecordSet(
								new ResourceRecordSet().withName(hostname).withType("A").withTTL(60L).withResourceRecords(new ResourceRecord().withValue(ipAddress)))));
				Future<ChangeResourceRecordSetsResult> future = amazonRoute53Async.changeResourceRecordSetsAsync(changeResourceRecordSetsRequest);
				future.get();
			}
